...
//    private UserPaging userPaging;
	...
	public String isExistId(String id) {
		// findById()는 기본 함수 => 반환값이 optional (null값 일수도 있다.) => Optional<반환객체>로 받아야한다.
		Optional<UserDTO> userDTO = userDAO.findById(id);
		System.out.println("userDTO = " + userDTO);
		if(userDTO.isPresent()) {     // 값이 없을 때(null일때) false를 반환하는 Optional 객체 유무 체크 함수
			return "exist";
		}else{
			return "non_exist";
		}
